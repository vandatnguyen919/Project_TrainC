...
void loadAndPrintImportDatafromFile(Product products[], int *count);
void loadAndPrintExportDatafromFile(Product products[], int *count);
...
int main() {
	...
	loadAndPrintImportDataFromFile("import.txt");
	loadAndPrintExportDataFromFile("export.txt");
...
void loadAndPrintImportDataFromFile(const char* filename) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (sscanf(line, "| %d | %10s | %18d | %11s | %19d | %21d | %[^\n]", &no, id, &total_qty, iprice, &imp_qty, &total_money, import_time) == 7) {
            printf("%-7d %-10s %-18d %-11s %-19d %-21d %s\n", no, id, total_qty, iprice, imp_qty, total_money, import_time);
            row++;
        }
    }
...
void loadAndPrintExportDataFromFile(const char* filename) {
    FILE* fp = fopen(filename, "r");
    if (fp == NULL) {
        printf("Cannot open file %s\n", filename);
        return;
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (sscanf(line, "| %d | %10s | %18d | %11s | %19d | %21d | %[^\n]", &no, id, &total_qty, eprice, &exp_qty, &total_money, export_time) == 7) {
            printf("%-7d %-10s %-18d %-11s %-19d %-21d %s\n", no, id, total_qty, eprice, exp_qty, total_money, export_time);
            row++;
        }
    }
