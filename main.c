...
void storeFromAccountFile(Account* account, int* num_account) {   
    ...
    *num_account = i;
    ...
    int t;
    char s[LL];
    for (int k = 0; k < i; k++) {       // quét qua tất cả tài khoản
        strcpy(s, account[k].name);
        t=0;
        for (int m = 0; t <= 2; m++) {     // quét qua tất cả kí tự của từng tài khoản
            if(isspace(s[m]))   
                t++;
            if(t == 2) {
                s[m] = '\0';
...
void addNewAccount(Account* account, int* num_account) {
    ...
    strcpy(account[*num_account-1].username, inputUsername);
    strcpy(account[*num_account-1].password, inputPassword);
    strcpy(account[*num_account-1].name, inputFullName);
    strcpy(account[*num_account-1].gender, inputGender);
    strcpy(account[*num_account-1].dob, inputDOB);
    strcpy(account[*num_account-1].position, inputPosition);
